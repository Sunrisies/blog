...
// XSS和转义符
export const XSS_encode_html = (str: any) => {
  return str
 ...
 * 自动生成id
 */
export const createUid = (prefix:any) => {
...
export const changeTreeData = (list: any[]) => {
  ...
  topList.forEach((v) => {
    const ind = newList1.findIndex((v1:any) => v1.year === v.year);
    if (ind >= 0) {
  ...
  normalList.forEach((v) => {
    const ind = newList2.findIndex((v1:any) => v1.year === v.year);
    if (ind >= 0) {
    ...
    .then((data) => {
      ...
      parsedLyrics.forEach((v) => {
        if (v.timeList.length) {
          newParsedLyrics = [
            ...newParsedLyrics,
            ...v.timeList.map((v1:any) => ({ time: v1, text: v.text })),
          ];
        } else {
